...
app.get("/secrets", async (req, res) => {
  if (req.isAuthenticated()) {
    let secret = "Submit a secret! ";
    const result = await db.query('select secret from userInfo where email = $1',[req.user.email]);
...
app.post("/submit", async (req, res) => {
  if(req.isAuthenticated()){
    console.log("The req is : ",req.body.secret);
    res.redirect("/secrets");
    const result = await db.query('update userInfo set secret = $1 where email = $2 returning *',[req.body.secret,req.user.email]);
  } else {
    res.redirect("/login");
  }
})
